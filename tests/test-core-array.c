...
static void TEST_ArrayAsParams(char 	array[], 
							   uint32 	array_capacity, 
							   uint32 	array_length)
{
	CSafeArray_From(array, array_capacity, array_length);


	CORE_Assert(CSafeArray_Capacity(array) == array_capacity);
	CORE_Assert(CSafeArray_Size(array) == array_length);
}
...
static void TEST_1(void)
{
	...
	CORE_Assert(CORE_StrEqual(char_array, char_array_copy));
